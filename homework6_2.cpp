...
void huffman_traversal(TreeNode *node,int trace[],int top)
{
	
	int i,j;
	if(node->l != NULL){
		trace[top] = 0;
		huffman_traversal(node->l,trace,top+1);
	}
	if(node->r != NULL){
		trace[top] = 1;
		huffman_traversal(node->r,trace,top+1);
	}
...
void huffman_encoding(char *str, bits_stream *bits_str)
{
	...
	for(i=0;i<strlen(str);i++){
		...
		for(j=0;j<m_bit_size[k];j++,top++)
		bits_str->stream[top]=m_LUT[k][j];
		interval += m_bit_size[k];
	}
